...
function testEnd($result = null, $ready = false)
{
    if ($ready) {
        ...
        echo sprintf("\tTime: %.4f sec", microtime(true) - $GLOBALS['runTime']);
    }
    echo
            sprintf("\tMemory: %.2f MB", memory_get_usage() / 1024 / 1024 - $GLOBALS['startMem']) .
            sprintf("\tMemory peak: %.2f MB", memory_get_peak_usage() / 1024 / 1024) .
